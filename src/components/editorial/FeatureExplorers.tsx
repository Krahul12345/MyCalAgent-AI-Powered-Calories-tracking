...
export function MealWalkthrough() {
  ...
  return <div><div className={styles.chapterTabs} role="tablist" aria-label="Meal logging walkthrough">{steps.map((item, i) => <button key={item.label} id={`step-${i}`} role="tab" aria-selected={selected === i} aria-controls="step-panel" tabIndex={selected === i ? 0 : -1} onClick={() => setSelected(i)} onKeyDown={event => nextTab(event, i, steps.length, "step", setSelected)}><span>0{i + 1}</span>{item.label}</button>)}</div><div id="step-panel" role="tabpanel" aria-labelledby={`step-${selected}`}><AnimatePresence mode="wait" initial={false}><motion.div key={selected} className={styles.chapterBody} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: reduced ? 0 : .15 }}><figure className={styles.chapterFigure}><div className={styles.photo}><Image src={step.image} alt={step.alt} fill sizes="(max-width:640px) 90vw, 550px" /></div><figcaption className={styles.photoNote}>Illustrative lifestyle image, not an app screenshot.</figcaption></figure><div className={styles.chapterCopy}><span className={styles.eyebrow}>0{selected + 1} / {step.label.toUpperCase()}</span><h3>{step.title}</h3><p>{step.body}</p><ul>{step.points.map(point => <li key={point}><Check size={17} aria-hidden="true" />{point}</li>)}</ul></div></motion.div></AnimatePresence></div></div>;
}
